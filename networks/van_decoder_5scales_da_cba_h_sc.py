...
class ChannelAttention(nn.Module):
    def __init__(self, in_planes, out_planes, ratio=16):
        ...
        self.fc = nn.Sequential(
            nn.Linear(out_planes,out_planes // ratio, bias = False),
            nn.ReLU(inplace = True),
            nn.Linear(out_planes // ratio, out_planes, bias = False)
            )
        '''
        self.fc = nn.Sequential(
    ...
    def forward(self, in_feature):
        ...
        return self.act3(self.conv3(self.sigmoid(avg_out).expand_as(x1) * in_feature+in_feature))
        
    

class VANDecoder(nn.Module):
    def __init__(self, num_ch_enc, scales=range(5), height=192, width=640, num_output_channels=1, use_skips=True):
        ...
        for s in range(len(scales)-1, -1, -1):
